...
export async function GET() {
  try {
    ...
    if (!userId) {
      console.error('API History - Error: User ID not found in session', JSON.stringify(session.user, null, 2));
      return Response.json({ error: 'User ID missing in session' }, { status: 500 });
    }
